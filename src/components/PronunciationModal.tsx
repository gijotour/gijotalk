...
import { startPronunciationCheck, speakPhrase } from '../utils/audio';
      ...
      <div className="bg-white border-2 border-orange-100 rounded-3xl max-w-md w-full p-6 text-slate-900 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 text-center">
        ...
        <div className="flex justify-between items-center pb-3 border-b border-orange-100 mb-4">
          <div className="flex items-center gap-2">
            <Mic className="w-5 h-5 text-[#FF6B4A]" />
            <h3 className="font-black text-base text-slate-900">실전 발음 체크 (STT)</h3>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Phrase Info */}
        <div className="bg-[#FDFBF7] p-4 rounded-2xl border-2 border-slate-100 mb-6 shadow-xs">
          <p className="text-[#FF6B4A] font-black text-2xl italic font-['Montserrat']">
            {phrase.original}
          </p>
          <p className="text-sm font-bold text-slate-800 mt-0.5">{phrase.translation}</p>
          <p className="text-xs font-black text-[#D97706] mt-1">발음: {phrase.pronunciation}</p>
        </div>
          ...
          {isListening ? (
            <div className="flex flex-col items-center gap-3">
              <div className="relative flex items-center justify-center">
                <div className="w-20 h-20 rounded-full bg-[#FF6B4A]/20 animate-ping absolute"></div>
                <div className="w-16 h-16 rounded-full bg-[#FF6B4A] flex items-center justify-center text-white shadow-lg shadow-[#FF6B4A]/40 z-10">
                  <Mic className="w-8 h-8 animate-pulse" />
                </div>
              </div>
              <p className="text-xs font-black text-[#FF6B4A] animate-pulse">
                듣고 있습니다... 크게 발음해보세요!
              </p>
            </div>
          ) : score !== null ? (
            <div className="space-y-3 w-full animate-in fade-in duration-200">
              <div className="flex items-center justify-center gap-2 text-3xl font-black">
                <span className={score >= 60 ? 'text-emerald-600' : 'text-[#D97706]'}>
                  {score}점
                </span>
                {score >= 60 ? (
              ...
              <div className="bg-orange-50/60 p-3 rounded-xl border border-orange-200 text-xs text-slate-700">
                <span className="text-slate-400 block text-[10px] font-bold">인식된 음성:</span>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            onClick={() =>
              speakPhrase({ text: phrase.original, langCode: country.langCode })
            }
            className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 font-extrabold rounded-xl text-xs flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="flex-1 py-3 bg-[#FF6B4A] hover:bg-[#ff5a36] text-white font-extrabold rounded-xl text-xs flex items-center justify-center gap-2 shadow-md shadow-[#FF6B4A]/20 disabled:opacity-50"
          >
